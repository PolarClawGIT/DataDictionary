...
namespace DataDictionary.Main
{
    partial class Main : ApplicationBase
    {
        ...
        public Main() : base()
        {
            ...
            menuStrip.SendToBack(); // Move the menu to the top of the screen, above the Tool Strip created by ApplicationBase

            toolStrip.Items.Insert(0,
                new ToolStripButton(
                    "Model",
                    Resources.SoftwareDefinitionModel,
                    ManageModelCommand_ButtonClick)
                {
                    ToolTipText = "Model Manager",
                    DisplayStyle = ToolStripItemDisplayStyle.Image,
                });

            toolStrip.Items.Insert(1,
                new ToolStripSplitButton(
                    "Attribute",
                    Resources.NewAttribute,
                    NewAttributeCommand_ButtonClick)
                {
                    ToolTipText = "new Attribute",
                    DisplayStyle = ToolStripItemDisplayStyle.Image,
                    DropDown = attributeContextMenu
                });

            toolStrip.Items.Insert(2,
                new ToolStripSplitButton(
                    "Entity",
                    Resources.NewEntity,
                    NewEntityCommand_ButtonClick)
                {
                    ToolTipText = "new Entity",
                    DisplayStyle = ToolStripItemDisplayStyle.Image,
                    DropDown = entityContextMenu
                });

            toolStrip.Items.Insert(3,
                new ToolStripSplitButton(
                    "Subject",
                    Resources.NewDiagram,
                    NewSubjectAreaCommand_ButtonClick)
                {
                    ToolTipText = "new Subject",
                    DisplayStyle = ToolStripItemDisplayStyle.Image,
                    DropDown = subjectAreaContextMenu
                });

            toolStrip.Items.Insert(4, new ToolStripSeparator());

            toolStrip.Items.Insert(5,
                new ToolStripSplitButton(
                    "Database",
                    Resources.Database,
                    manageDatabasesCommand_ButtonClick)
                {
                    ToolTipText = "Database Manager",
                    DisplayStyle = ToolStripItemDisplayStyle.Image,
                    DropDown = catalogContextMenu
                });

            toolStrip.Items.Insert(6,
                new ToolStripSplitButton(
                    "Library",
                    Resources.Library,
                    manageLibrariesCommand_ButtonClick)
                {
                    ToolTipText = "Library Manager",
                    DisplayStyle = ToolStripItemDisplayStyle.Image,
                    DropDown = libraryContextMenu
                });

            toolStrip.Items.Insert(7, new ToolStripSeparator());

            toolStrip.Items.Insert(8,
                new ToolStripButton(
                    "Refresh",
                    Resources.Refresh,
                    RefreshCommand_Click)
                {
                    ToolTipText = "Refresh navigation tree",
                    DisplayStyle = ToolStripItemDisplayStyle.Image,
                });
