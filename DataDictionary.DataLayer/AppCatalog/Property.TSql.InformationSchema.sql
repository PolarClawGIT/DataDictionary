With [Parameter] As (
	...
	Union
	Select	'SCHEMA' As [Level0Type],
			[TABLE_SCHEMA] As [Level0Name],
			'TABLE' As [Level1Type],
			[TABLE_NAME] As [Level1Name],
			Null As [Level2Type],
			Null As [Level2Name]
	...
	Union
	Select	'SCHEMA' As [Level0Type],
			C.[TABLE_SCHEMA] As [Level0Name],
			'TABLE' As [Level1Type],
			C.[TABLE_NAME] As [Level1Name],
	...
	Union
	Select	'SCHEMA' As [Level0Type],
			[ROUTINE_SCHEMA] As [Level0Name],
			[ROUTINE_TYPE] As [Level1Type],
			[ROUTINE_NAME] As [Level1Name],
			Null As [Level2Type],
			Null As [Level2Name]
	...
	Select	'SCHEMA' As [Level0Type],
			...
			IIF([IS_RESULT] = 'YES' And NullIf([PARAMETER_NAME],'') is Null, 'RETURN', [PARAMETER_NAME]) As [Level2Name]
	From	[INFORMATION_SCHEMA].[ROUTINES] R
			INNER Join [INFORMATION_SCHEMA].[PARAMETERS] P
			On	R.[SPECIFIC_CATALOG] = P.[SPECIFIC_CATALOG] And
	...
	Select	'SCHEMA' As [Level0Type],
			...
			[ROUTINE_NAME] As [Level1Name],
			'COLUMN' As [Level2Type],
			[COLUMN_NAME] As [Level2Name]
	From	[INFORMATION_SCHEMA].[ROUTINES] R
			INNER Join [INFORMATION_SCHEMA].[ROUTINE_COLUMNS] C
			On	R.[SPECIFIC_CATALOG] = C.[TABLE_CATALOG] And
				R.[SPECIFIC_SCHEMA] = C.[TABLE_SCHEMA] And
				R.[SPECIFIC_NAME] = C.[TABLE_NAME]),
[Exceptions] As (
	Select	Convert(SysName,Null) As [Level0Name],
			Convert(SysName,Null) As [Level1Name],
			Convert(SysName,Null) As [Level2Name]
	Where 1=2
