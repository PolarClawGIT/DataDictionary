...
namespace DataDictionary.DataLayer.AppCatalog
{
        ...
        where TItem : ConstraintColumnItem, IConstraintColumnItem, new()
    {
        ...
        /// <inheritdoc/>
        public Command LoadCommand(IConnection connection, IConstraintKey constraintKey, ITemporalKey asOfUtcDate)
        { return LoadCommand(connection, constraintId: constraintKey.ConstraintId, asOfUtcDate: asOfUtcDate.AsOfUtcDate); }
        ...
        Command SaveCommand(IConnection connection, Guid? catalogId = null, Guid? constraintId = null)
        {
            ...
            command.AddParameter(WriteData.Data, ConstraintColumn.TableType, data);
            return command;
        }
        ...
        /// <inheritdoc/>
        public void Import(ICatalogKey catalogKey, IEnumerable<IConstraintColumn> source)
        {
            ...
            foreach (var key in allKeys)
            {
                ...
                IConstraintColumn? newValue = source.FirstOrDefault(w => key.Equals(w));

                if (oldValue is ConstraintColumnItem oldMatches && newValue is IConstraintColumn newMatches)
                { oldMatches.Update(newMatches); } // Update Old
                else if (oldValue is ConstraintColumnItem newMissing)
                { this.Remove(key); } // Delete Old
                else if (newValue is IConstraintColumn oldMissing)
                { Add(ConstraintColumnItem.Create<TItem>(catalogKey, oldMissing)); }// Add New
            }
        }
    }
}
