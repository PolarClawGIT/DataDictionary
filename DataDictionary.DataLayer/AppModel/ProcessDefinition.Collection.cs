...
{
    /// <summary>
    /// Generic Base class for Model Process Definitions
    /// </summary>
    /// <typeparam name="TItem"></typeparam>
    /// <remarks>Base class, implements the Read and Write.</remarks>
    public abstract class ProcessDefinitionCollection<TItem> : BindingTable<TItem>,
        ...
        where TItem : ProcessDefinitionItem, new()
    {
        /// <inheritdoc/>
            ...
            DateTime? asOfUtcDate = null, Boolean includeHistory = false)
        {
            ...
            command.CommandText = ProcessDefinition.GetProcedure;
        ...
        Command SaveCommand(IConnection connection, Guid? modelId = null, Guid? ProcessId = null)
        {
            Command command = connection.CreateCommand();
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = ProcessDefinition.SetProcedure;
            command.AddParameter(Model.ModelId, modelId);
            command.AddParameter(Process.ProcessId, ProcessId);

            IEnumerable<TItem> data = this.Where(w =>
                (ProcessId is null || w.ProcessId == ProcessId));
            command.AddParameter(WriteData.Data, ProcessDefinition.TableType, data);
            return command;
        }
