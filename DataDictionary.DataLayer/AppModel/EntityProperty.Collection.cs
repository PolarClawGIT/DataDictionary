...
{
    /// <summary>
    /// Generic Base class for Model Entity Properties
    /// </summary>
    /// <typeparam name="TItem"></typeparam>
    /// <remarks>Base class, implements the Read and Write.</remarks>
    public abstract class EntityPropertyCollection<TItem> : BindingTable<TItem>,
        ...
        where TItem : BindingTableRow, IEntityPropertyItem, new()
    {
        /// <inheritdoc/>
            ...
            DateTime? asOfUtcDate = null, Boolean includeHistory = false)
        {
            ...
            command.CommandText = EntityProperty.GetProcedure;
        ...
        Command SaveCommand(IConnection connection, Guid? modelId = null, Guid? entityId = null)
        {
            Command command = connection.CreateCommand();
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = EntityProperty.SetProcedure;
            command.AddParameter(Model.ModelId, modelId);
            command.AddParameter(Entity.EntityId, entityId);

            IEnumerable<TItem> data = this.Where(w =>
                (entityId is null || w.EntityId == entityId));
            command.AddParameter(WriteData.Data, EntityProperty.TableType, data);
            return command;
        }
