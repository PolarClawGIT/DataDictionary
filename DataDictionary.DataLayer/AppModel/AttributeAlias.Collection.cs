...
namespace DataDictionary.DataLayer.AppModel
{
    ...
    public abstract class AttributeAliasCollection<TItem> : BindingTable<TItem>,
        ...
        where TItem : AttributeAliasItem, new()
    {
        /// <inheritdoc/>
            ...
            DateTime? asOfUtcDate = null, Boolean includeHistory = false)
        {
            ...
            command.CommandText = AttributeAlias.GetProcedure;
        ...
        Command SaveCommand(IConnection connection, Guid? modelId = null, Guid? attributeId = null)
        {
            Command command = connection.CreateCommand();
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = AttributeAlias.SetProcedure;
            command.AddParameter(Model.Identifier, modelId);
            command.AddParameter(Attribute.Identifier, attributeId);

            IEnumerable<TItem> data = this.Where(w =>
                (attributeId is null || w.AttributeId == attributeId));
            command.AddParameter(WriteData.Data, AttributeAlias.TableType, data);
            return command;
        }
