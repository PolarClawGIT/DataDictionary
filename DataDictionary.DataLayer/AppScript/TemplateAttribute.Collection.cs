...
namespace DataDictionary.DataLayer.AppScript
{
    ...
    public class TemplateAttributeCollection<TItem> : BindingTable<TItem>,
        ...
        where TItem : BindingTableRow, ITemplateAttributeItem, new()
    {
        /// <inheritdoc/>
            ...
            DateTime? asOfUtcDate = null, Boolean includeHistory = false)
        {
            ...
            command.CommandText = TemplateAttribute.GetProcedure;
        ...
        private Command SaveCommand(IConnection connection, Guid? modelId = null, Guid? templateId = null)
        {
            Command command = connection.CreateCommand();
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = TemplateAttribute.SetProcedure;
            command.AddParameter(Model.ModelId, modelId);
            command.AddParameter(Template.TemplateId, templateId);

            IEnumerable<TItem> data = this.Where(w =>
                (templateId is null || w.TemplateId == templateId));
            command.AddParameter(WriteData.Data, TemplateAttribute.TableType, data);
            return command;
        }
