...
{
    /// <summary>
    /// Generic Base class for Scripting Transform Document
    /// </summary>
    /// <typeparam name="TItem"></typeparam>
    public class TransformDocumentCollection<TItem> : BindingTable<TItem>,
        IReadData, IReadData<IModelKey>, IReadData<ITemplateKey>,
        IWriteData<IModelKey>, IWriteData<ITemplateKey>,
        IRemoveItem<ITemplateKey>, IRemoveItem<ITransformKey>, IRemoveItem<IDocumentKey>,
        IReadTemporal<IModelKey>, IReadTemporal<ITemplateKey>
        where TItem : BindingTableRow, ITransformDocumentItem, new()
    {
        /// <inheritdoc/>
            ...
            DateTime? asOfUtcDate = null, Boolean includeHistory = false)
        {
            ...
            command.CommandText = TransformDocument.GetProcedure;
        ...
        private Command SaveCommand(IConnection connection, Guid? modelId = null, Guid? TemplateId = null)
        {
            Command command = connection.CreateCommand();
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = TransformDocument.SetProcedure;
            command.AddParameter(Model.Identifier, modelId);
            command.AddParameter(Template.Identifier, TemplateId);

            IEnumerable<TItem> data = this.Where(w =>
                (TemplateId is null || w.TemplateId == TemplateId));
            command.AddParameter(WriteData.Data, TransformDocument.TableType, data);
            return command;
        }
        ...
        /// <inheritdoc/>
        public void Remove(IDocumentKey documentKey)
        {
