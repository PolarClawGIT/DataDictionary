...
{
    /// <summary>
    /// Generic Base class for Scripting SchemaNodeOwner
    /// </summary>
    /// <typeparam name="TItem"></typeparam>
    public class SchemaNodeOwnerCollection<TItem> : BindingTable<TItem>,
        ...
        where TItem : BindingTableRow, ISchemaNodeOwnerItem, new()
    {
        /// <inheritdoc/>
            ...
            DateTime? asOfUtcDate = null, Boolean includeHistory = false)
        {
            ...
            command.CommandText = SchemaNodeOwner.GetProcedure;
        ...
        private Command SaveCommand(IConnection connection, Guid? modelId = null, Guid? TemplateId = null)
        {
            Command command = connection.CreateCommand();
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = SchemaNodeOwner.SetProcedure;
            command.AddParameter(Model.Identifier, modelId);
            command.AddParameter(Template.Identifier, TemplateId);

            IEnumerable<TItem> data = this.Where(w =>
                (TemplateId is null || w.TemplateId == TemplateId));
            command.AddParameter(WriteData.Data, SchemaNodeOwner.TableType, data);
            return command;
        }
