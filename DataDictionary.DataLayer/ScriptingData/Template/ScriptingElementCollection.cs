...
namespace DataDictionary.DataLayer.ScriptingData.Template
{
    ...
    public abstract class ScriptingElementCollection<TItem> : BindingTable<TItem>,
        ...
        where TItem : BindingTableRow, IScriptingElementItem, new()
    {
        /// <inheritdoc/>
        ...
        private Command LoadCommand(IConnection connection, (Guid? modelId, Guid? templateId) parameters)
        {
            ...
            command.CommandText = "[App_DataDictionary].[procGetScriptingElement]";
        ...
        private Command SaveCommand(IConnection connection, (Guid? modelId, Guid? templateId) parameters)
        {
            Command command = connection.CreateCommand();
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = "[App_DataDictionary].[procSetScriptingElement]";
            command.AddParameter("@ModelId", parameters.modelId);
            command.AddParameter("@TemplateId", parameters.templateId);

            IEnumerable<TItem> data = this.Where(w => parameters.templateId is null || w.TemplateId == parameters.templateId);
            command.AddParameter("@Data", "[App_DataDictionary].[typeScriptingElement]", data);
