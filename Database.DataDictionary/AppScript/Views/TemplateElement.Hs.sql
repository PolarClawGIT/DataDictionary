CREATE VIEW [AppScript].[TemplateElementHs] As
-- Temporal View
With [Dates] As (
	Select	[ElementId],
			[SysStart],
	...
	Select	[ElementId],
			[SysStart],
			[SysEnd]
	From	[HsScript].[ScriptingElement]
	Where	[SysStart] != [SysEnd]*/)
Select	D.[ElementId], -- PK
		D.[TemplateId], -- AK
		FT.[TemplateTitle],
		D.[ElementName],  -- AK
		D.[ParentElementId], -- AK
		D.[RenderOrder],
		D.[RenderValueAs],
		D.[ObjectScope],
		D.[ObjectProperty],
...
From	[AppScript].[TemplateElement] D
		Outer Apply (
			Select	Max([SysEnd]) As [PriorDate]
			From	[Dates]
			Where	[ElementId] = D.[ElementId] And
