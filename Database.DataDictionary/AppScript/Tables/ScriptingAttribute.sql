CREATE TABLE [AppScript].[ScriptingAttribute]
(	-- Key/Value pairs to add as Attributes to an XML Node.
	...
	[ModifiedBy] SysName Not Null CONSTRAINT [DF_ScriptingNodeAttribute_ModifiedBy] DEFAULT (original_login()),
	...
	CONSTRAINT [FK_ScriptingNodeAttribute] FOREIGN KEY ([NodeId]) REFERENCES [AppScript].[ScriptingNode] ([NodeId]),
...
CREATE UNIQUE NONCLUSTERED INDEX [UX_ScriptingNodeAttribute]
    ON [AppScript].[ScriptingAttribute]([NodeId], [AttributeName] ASC);
GO