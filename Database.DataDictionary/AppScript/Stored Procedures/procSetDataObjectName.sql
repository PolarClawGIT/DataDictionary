CREATE PROCEDURE [AppScript].[procSetDataObjectName]
		@ModelId UniqueIdentifier = Null,
		@DataSourceId UniqueIdentifier = Null,
		@Data [AppScript].[udttDataObjectName] ReadOnly
As
...
Begin Try
	...
	;With [Data] As (
		...
		From	@Data D
				...
				Cross Apply (
					Select	IsNull([AppScript].[funcObjectNameId]([QualifiedName]), NewId()) As [ObjectNameId]) X)
	Insert Into @Values
	Select	[DataSourceId],
			Coalesce([AppScript].[funcObjectNameId]([ObjectNameSpace]), [ObjectNameId], NewId()) As [ObjectNameId],
			[ObjectNameMember],
			Null,
			[ObjectNameSpace],
			[ParentNameSpace]
	...
	-- Apply Changes
	Insert Into [AppScript].[DataObjectName] (
			[DataSourceId],
			[ObjectNameId],
			[ObjectMember],
			[ParentNameId])
	...
	From	@Values S
			...
			Left Join [AppScript].[DataObjectName] T
