CREATE PROCEDURE [AppLibrary].[procSetLibrarySource]
		...
		@Data [AppLibrary].[typeLibrarySource] ReadOnly
As
...
Begin Try
	...
	Select	Coalesce(A.[LibraryId], D.[LibraryId], @LibraryId, NewId()) As [LibraryId],
			...
			D.[SourceDate]
	From	@Data D
			Left Join [AppLibrary].[LibrarySource] A
	...
	Where	S.[LibraryId] is Null And
			M.[LibraryId] In (
				Select	A.[LibraryId]
				From	[AppLibrary].[LibrarySource] A
						Left Join [AppLibrary].[LibraryModel] M
	...
	Where	S.[LibraryId] is Null And
			...
			Select	A.[LibraryId]
			From	[AppLibrary].[LibrarySource] A
					Left Join [AppLibrary].[LibraryModel] C
	...
	Update [AppLibrary].[LibrarySource]
	...
	Select	@ModelId As [ModelId],
			S.[LibraryId]
	from	@Values S
			Left Join [AppLibrary].[LibraryModel] T
