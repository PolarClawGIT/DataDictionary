CREATE TABLE [AppLibrary].[LibraryMember]
(	[MemberId]              UniqueIdentifier Not Null CONSTRAINT [DF_LibraryMemberId] DEFAULT (newid()),
	...
	[MemberType]            [AppGeneral].[uddtObjectType] Null, -- .Net Documents produce a letter that is converted to a Member type. There may not be one.
	...
	CONSTRAINT [FK_LibraryMemberSource] FOREIGN KEY ([LibraryId]) REFERENCES [AppLibrary].[LibrarySource] ([LibraryId]),
	CONSTRAINT [FK_LibraryMemberParent] FOREIGN KEY ([MemberParentId]) REFERENCES [AppLibrary].[LibraryMember] ([MemberId]),
...
CREATE NONCLUSTERED INDEX [FK_LibraryMemberParent]
    ON [AppLibrary].[LibraryMember]([MemberParentId]);
GO