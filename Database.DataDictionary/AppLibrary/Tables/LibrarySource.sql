CREATE TABLE [AppLibrary].[LibrarySource]
(
	...
	[LibraryTitle]          [AppGeneral].[typeTitle] Not Null,-- expected to be the same as [AssemblyName], but can be changed.
...
CREATE UNIQUE NONCLUSTERED INDEX [UX_LibrarySourceAssembly]
    ON [AppLibrary].[LibrarySource]([AssemblyName]);
GO
CREATE UNIQUE NONCLUSTERED INDEX [UX_LibrarySourceTitle]
    ON [AppLibrary].[LibrarySource]([LibraryTitle]);
GO