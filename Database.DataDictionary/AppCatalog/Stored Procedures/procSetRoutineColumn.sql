...
Begin Try
	...
	From	@Data D
			...
			Left Join [AppCatalog].[RoutineColumnHs] A
	...
	Where	S.[ColumnId] is Null And
			P.[CatalogId] In (
				...
				Where	(@CatalogId is Null Or @CatalogId = A.[CatalogId]) And
						(@ModelId is Null Or @ModelId = C.[ModelId]))
	Print FormatMessage ('Delete [App_DataDictionary].[DatabaseRoutineColumn]: %i, %s',@@RowCount, Convert(VarChar,GetDate()));
	...
	From	[AppCatalog].[RoutineColumn] T
			Inner Join [Delta] S
			On	T.[ColumnId] = S.[ColumnId]
	Print FormatMessage ('Update [App_DataDictionary].[DatabaseRoutineColumn]: %i, %s',@@RowCount, Convert(VarChar,GetDate()));
	...
	Print FormatMessage ('Insert [App_DataDictionary].[DatabaseRoutineColumn]: %i, %s',@@RowCount, Convert(VarChar,GetDate()));
