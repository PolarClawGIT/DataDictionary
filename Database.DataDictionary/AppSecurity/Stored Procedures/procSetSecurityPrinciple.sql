CREATE PROCEDURE [AppSecurity].[procSetSecurityPrinciple]
		@PrincipleId UniqueIdentifier = Null,
		@Data [AppSecurity].[typeSecurityPrinciple] ReadOnly
As
Set NoCount On -- Do not show record counts
Set XACT_ABORT On -- Error severity of 11 and above causes XAct_State() = -1 and a rollback must be issued
/* Description: Performs Set on SecurityPrinciple.
*/
...
Begin Try
	...
	Delete From [AppSecurity].[SecurityPrinciple]
	From	[AppSecurity].[SecurityPrinciple] T
			Left Join @Values S
			On	T.[PrincipleId] = S.[PrincipleId]
	Where	S.[PrincipleId] is Null And
			(@PrincipleId is Null or @PrincipleId = T.[PrincipleId])
	...
	Update [AppSecurity].[SecurityPrinciple]
	...
	Insert Into [AppSecurity].[SecurityPrinciple] (
			[PrincipleId],
			[PrincipleLogin],
			[PrincipleName],
			[PrincipleAnnotation])
	...
	From	@Values S
			Left Join [AppSecurity].[SecurityPrinciple] T
