CREATE FUNCTION [AppSecurity].[funcModelAuthorization](
...
With [Authorization] As (
	Select	[PrincipalLogin],
			...
			Convert(Bit, Case
				...
				Else 0 End)
				As [IsAuthorized]
	From	[AppSecurity].[funcAuthorization](@ModelId))
Select	[PrincipalLogin],
		[PrincipalId],
		[IsApplication],
		[IsDbWriter],
		...
		[IsAuthorized]
From	[Authorization]
Where	(@ModelId is Null And @IsAuthorized is Null And ([IsApplication] = 1 or [IsDbWriter] = 1)) Or
