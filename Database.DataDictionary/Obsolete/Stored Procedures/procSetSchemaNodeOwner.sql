CREATE PROCEDURE [AppScript].[procSetSchemaNodeOwner]
		@ModelId UniqueIdentifier = Null,
		@TemplateId UniqueIdentifier = Null,
		@Data [AppScript].[udttSchemaNodeOwner] ReadOnly
AS
...
Begin Try
	...
	Declare @Values Table (
		...
		Primary Key([NodeId], [NodeOwnerId]))
	...
	-- Apply Changes
	Delete From [AppScript].[SchemaNodeOwner]
	From	[AppScript].[SchemaNodeOwner] T
	...
	Print FormatMessage ('Delete [AppScript].[SchemaNodeOwner]: %i, %s', @@RowCount, Convert(VarChar,GetDate()));

	Insert Into [AppScript].[SchemaNodeOwner] (
			[NodeId],
			[NodeOwnerId],
			[SchemaId])
	Select	S.[NodeId],
	...
	From	@Values S
			...
			Left Join [AppScript].[SchemaNodeOwner] T
			On	S.[SchemaId] = T.[SchemaId]
			Cross Apply [AppSecurity].[funcScriptingAuthorization](F.[TemplateId], 1)
	Where	T.[SchemaId] is Null
	Print FormatMessage ('Insert [AppScript].[SchemaNodeOwner]: %i, %s',@@RowCount, Convert(VarChar,GetDate()));
